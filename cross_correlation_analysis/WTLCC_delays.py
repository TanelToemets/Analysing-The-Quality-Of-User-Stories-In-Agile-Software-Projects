...
#Read nr of delays
delays_df = pd.read_csv("C:/Users/Tanel/Documents/Ylikool/Magister/Master Thesis/Analysing ASP Repo/data/WTLCC_input_data/{0}-delays-for-WTLCC.csv".format(project))
print('len of delays file')
print(len(delays_df))
...
#delays_df = manage_compass_field_names(delays_df, project)
# print(quality_scores_df)
# print(delays_df)
delays_df = delays_df.rename(columns={'fields.resolutiondate': 'fields.created'})
...
delays_df['fields.created'] = pd.to_datetime(delays_df['fields.created'], utc=True)
delays_df = delays_df.set_index(pd.DatetimeIndex(delays_df['fields.created']))
...
resampled_delays = delays_df.resample('B')['delay_nr'].sum()
plt.show()
...
#inner join on resampled quality and delays
quality_delays_df = pd.merge(resampled_quality, resampled_delays, left_index=True, right_index=True)
print('len of quality_delays_df')
print(len(quality_delays_df))
...
# print(quality_delays_df.index)
quality_delays_df = quality_delays_df.assign(InterpolateTime=quality_delays_df['quality'].interpolate(method='time'))
quality_delays_df = quality_delays_df.drop(['quality'], axis=1)
quality_delays_df = quality_delays_df.rename(columns={'InterpolateTime': 'quality'})
#If there was NaN values at the start of the time frame imputation was not possible, therefore NaN values will be dropped
quality_delays_df.dropna( inplace=True)
print(len(quality_delays_df))

# Test files
# quality_delays_df.to_csv("C:/Users/Tanel/Documents/Ylikool/Magister/Master Thesis/Analysing ASP Repo/test.csv", sep=',', encoding='utf-8', doublequote = True, header=True, index=False, line_terminator=",\n")
...
no_splits = int(proper_round(len(quality_delays_df)/66))    #SETTING 2 len/66B | Quarter | 1Q ~ 66B
#no_splits = int(proper_round(len(quality_delays_df)/22))    #SETTING 3 len/22B | Months  | 1M ~ 22B 
#no_splits = int(proper_round(len(quality_delays_df)/10))    #SETTING 4 len/10B | Sprint  |~2W = 10B 


#calculating cross correlation matrix
samples_per_split = quality_delays_df.shape[0]/no_splits  #samples_per_split = whole length of time series/time window  #673/4=168.25 
u = int(proper_round(samples_per_split/2)) #int(round(samples_per_split/2) #168/2=84
l = -u
rss=[]
for t in range(0, no_splits):
    quality_delays_df.reset_index()
    d1 = quality_delays_df['quality'].reset_index(drop=True).loc[(t)*samples_per_split:(t+1)*samples_per_split] #red on matrix shows that US quality is leading
    d2 = quality_delays_df['delay_nr'].reset_index(drop=True).loc[(t)*samples_per_split:(t+1)*samples_per_split] #blue then delay nr is leading
...
ax.set(title=f'WTLCC for user story quality and number of delays',xlim=[0,u+u], xlabel='Lag in business days',ylabel='Time window')
