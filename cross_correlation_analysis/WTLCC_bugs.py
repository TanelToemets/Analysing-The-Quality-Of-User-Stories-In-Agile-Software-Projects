...
bugs_df = choose_active_development_periods(bugs_df, project)
print(quality_scores_df)
print(bugs_df)
...
quality_bugs_df = pd.merge(resampled_quality, resampled_bugs, left_index=True, right_index=True)
...
ax.set(title=f'WTLCC for user story quality and number of bugs',xlim=[0,u+u], xlabel='Lag in business days',ylabel='Time window')
